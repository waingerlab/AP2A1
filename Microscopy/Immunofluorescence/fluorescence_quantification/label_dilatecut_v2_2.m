...
%version 2 adds a size filter based on nuclear:cytoplasmic ratio; v2_2
%increases ratio

%function
function [outputimage, dilatedmask, nuclearmask] = label_dilatecut_v2_1(labeledobjectmask,cutmask,growthcycles)
...
ratio(1)=[]; %delete 0 value
index(1)=[]; %delete 0 value

delete = double(index).*double(ratio>6);
delete(delete==0)=[];
delete2 = double(index).*double(ratio<1.8);
delete2(delete2==0)=[];
delete = [delete;delete2];

deletemask = ismember(dilatedmask,delete);
dilatedmask(deletemask)=0;
labeledobjectmask(deletemask)=0;
